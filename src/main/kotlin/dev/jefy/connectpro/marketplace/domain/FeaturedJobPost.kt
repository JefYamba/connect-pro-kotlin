 ...
 */
@Embeddable
data class FeaturedJobPostId(var value: UUID){
    companion object {
        fun generate(): FeaturedJobPostId = FeaturedJobPostId(UUID.randomUUID())
    }
}
...
open class FeaturedJobPost(
    ...
    open var id: FeaturedJobPostId = FeaturedJobPostId.generate()
        protected set

    @Embedded
    @AttributeOverride(name = "value", column = Column(name = "job_post_id"))
    open var jobPostId: JobPostId = jobPostId
        protected set
